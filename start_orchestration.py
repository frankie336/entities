...
class DockerOrchestrationManager:
    ...
    _COMPOSE_ENV_MAPPING = {
        ...
        "DEFAULT_SECRET_KEY": ("api", "DEFAULT_SECRET_KEY"),
        "SANDBOX_SERVER_URL": ("api", "SANDBOX_SERVER_URL"),
    ...
    def _get_host_port_from_compose_service(self, service_name, container_port):
        ...
        if not self.compose_config:
            self.log.warning(
                f"Compose config not loaded, cannot determine host port for {service_name}:{container_port}"
            )
            return None
        try:
            service_data = self.compose_config.get("services", {}).get(service_name)
            ...
            for port_mapping in ports:
                ...
                if len(parts) == 1:
                    ...
                    len(parts) >= 3
                ):  # Handles "ip:host:container" and potential future ipv6 formats
    ...
    def _generate_dot_env_file(self):
        ...
        if self.compose_config:
            ...
            self.log.warning(
                f"Cannot source values from {self._DOCKER_COMPOSE_FILE} as it wasn't loaded."
            )
        ...
        db_pass = env_values.get(
            "MYSQL_PASSWORD"
        )  # Password might be missing if not in compose/defaults
        db_host_internal = env_values.get(
        ...
        if all([db_user, db_pass is not None, db_host_internal, db_port_internal, db_name]):
            ...
            generation_log["DATABASE_URL"] = (
                f"Skipped: Missing DB components ({', '.join(missing_components)})"
            )
            generation_log["SPECIAL_DB_URL"] = (
                f"Skipped: Missing DB components ({', '.join(missing_components)})"
            )
    ...
    def _check_for_required_env_file(self):
        ...
        if not os.path.exists(self._ENV_FILE):
            self.log.warning(f"Required environment file '{self._ENV_FILE}' is missing.")
            self._generate_dot_env_file()
        else:
            self.log.info(f"Required environment file '{self._ENV_FILE}' exists. NOT overwriting.")
    ...
    def _start_ollama(self, cpu_only=True):
        ...
        try:
            ...
            if self._is_container_running(container_name):
                ...
                self.log.error(
                    f"❌ External Ollama container '{container_name}' failed to start. Check logs: docker logs {container_name}"
                )
                ...
                try:
                    ...
                    pass  # Ignore errors fetching logs here
                return False
        except Exception as e:
    ...
    def _ensure_ollama(self, opt_in=False, use_gpu=False):
        ...
        if os.path.exists("/.dockerenv") or os.environ.get("DOCKER_HOST"):
            self.log.warning(
                "🛰 Running inside Docker or remote daemon. Skipping external Ollama management."
            )
            return True
        if not self._has_docker():
        ...
        if platform.system() == "Darwin":
            self.log.warning(
                "⚠️ macOS detected. GPU passthrough may be limited. Native Ollama app recommended for GPU."
            )
        ...
        if use_gpu and not gpu_available and platform.system() != "Darwin":
            self.log.warning(
                "⚠️ GPU requested (--ollama-gpu), but unavailable/unsupported. Proceeding in CPU mode."
            )
        # Start Ollama
    ...
    def _handle_nuke(self):
        ...
        self.log.warning(
            "This will: 1) Stop/remove this project's containers/volumes. 2) Prune ALL unused Docker data (containers, networks, images, build cache, VOLUMES)."
        )
        try:
        ...
        try:
            ...
            self.log.warning(f"Error during 'compose down': {e}. Continuing nuke.")
        ...
        try:
            ...
            self.log.critical(f"Critical error during 'system prune': {e}")
            sys.exit(1)
    ...
    def _handle_down(self):
        ...
        if self.args.clear_volumes:
            ...
            try:
                ...
                self.log.error("Confirmation required. Aborting.")
                sys.exit(1)
            if confirm != "yes":
    ...
    def _handle_up(self):
        ...
        if not os.path.exists(self._ENV_FILE):
            self.log.error(
                f"Required environment file '{self._ENV_FILE}' is missing. Cannot start."
            )
            sys.exit(1)
        ...
        target_services = self.args.services or []
        target_desc = (
            f" for services: {', '.join(target_services)}" if target_services else " all services"
        )
...
if __name__ == "__main__":
    # Generate docker-compose.yml with unique values
