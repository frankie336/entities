...
try:
    ...
    try:
        ...
        print("Please ensure:")
        print("  1. The 'projectdavid' package is correctly installed within the 'api' container image.")
        print("  2. The script is run from a location where Python can find the installed packages (e.g., /app/scripts if WORKDIR=/app).")
        sys.exit(1)
...
def load_admin_key(env_var=ENV_VAR_ADMIN_KEY, creds_file=DEFAULT_CREDS_FILE):
    ...
    if not admin_api_key:
        creds_file_path = os.path.join(os.path.dirname(__file__), creds_file) # Assume creds file is sibling to script
            ...
            try:
                with open(creds_file_path, "r") as f:
                    for line in f:
                        ...
                        if line.startswith(f"{ENV_VAR_ADMIN_KEY}=") or line.startswith("ENTITIES_API_KEY="):
             ...
             print(f"{env_var} not found in env, and fallback credentials file '{creds_file_path}' not found.")
    ...
    print(f"Using Admin API Key (loaded from {source}) starting with: {admin_api_key[:4]}...{admin_api_key[-4:]}")
...
def create_user(client, full_name, email):
    ...
    try:
        ...
        if error_response is not None:
            ...
            try: error_detail = error_response.json()
            except Exception: error_detail = error_response.text
...
def generate_user_key(admin_client, user, key_name=DEFAULT_KEY_NAME):
    ...
    try:
        ...
        print(f"Calling SDK method 'create_key_for_user' on admin client for user ID {target_user_id}")
        ...
        if error_response is not None:
            ...
            print(f"Response Body: {error_detail}")
            if error_response.status_code == 404: print(f"Hint: Check API endpoint POST /v1/admin/users/{target_user_id}/keys")
            elif error_response.status_code == 403: print("Hint: Ensure the ADMIN_API_KEY has permission.")
            elif error_response.status_code == 422: print("Hint: Check the key_payload.")
        else: print(f"An unexpected error occurred: {key_gen_e}")
        return None
...
def main():
    ...
    parser.add_argument("-e", "--email", type=str, help="Email address for the new user. If omitted, a unique default is generated.")
    parser.add_argument("-n", "--name", type=str, help="Full name for the new user. If omitted, a default name is used.")
    parser.add_argument(
        "--base-url", type=str, default=os.getenv("ASSISTANTS_BASE_URL", DEFAULT_BASE_URL), # Prefer env var if set
        help=f"Base URL for the API. Defaults to ASSISTANTS_BASE_URL env var or {DEFAULT_BASE_URL}."
    )
    parser.add_argument(
        "--creds-file", type=str, default=DEFAULT_CREDS_FILE,
        help=f"Fallback path (relative to script) inside container for admin credentials file. Default: {DEFAULT_CREDS_FILE}"
    )
    parser.add_argument(
        "--key-name", type=str, default=DEFAULT_KEY_NAME,
